...
//forEach :In JavaScript, the forEach loop is a higher-order function that is used to iterate over elements of an array. It provides a concise way to perform an action for each element in the array without the need for explicit loop constructs like for or while loops. However, keep in mind that the forEach loop does not create a new array; it simply iterates over the existing one. If you need to create a modified array based on the elements of the original array, you might want to consider using methods like map.
...
function multiplyByTwo(number, index) {
  ...
  console.log(`Nnumber at index ${index} is ${number} multiply by 2 is,`, number * 2);
}
